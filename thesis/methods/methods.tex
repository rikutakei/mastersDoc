...
\gls{rnaseq} and clinical data of multiple different cancer types were downloaded from the \gls{icgc} (\citetalias{ICGC}; \citealp{Zhang2011}) and \gls{tcga} \citepalias{TCGA}, respectively.
...
% TODO: check the meaning of precision weight in the above sentence
...
%TODO: more details on simple Bayesian method
...
Self-contained tests are more appropriate in assessing whether a biological process is significantly involved in an expreiment, and in contrast, competitive tests are better for selecting the most relevant biological processes from those that are not \citep{Wu2012}.
...
\begin{figure}[htpb]
	...
	\caption[Example heatmaps showing the direction of the uncorrected and corrected \acrshort{er} pathway metagene with the \acrshort{er} gene expression in the \acrshort{rma}-normalised Gatza \textit{et al.} data.]{Heatmaps showing the direction of the unflipped (left) and flipped (right) \gls{er} pathway metagene with the \gls{er} gene expression in the \gls{rma}-normalised Gatza \textit{et al.} data.
	...
	The \gls{er} gene expressions of the samples are shown as a separate row at top of the heatmap with the sample \gls{er} metagene scores below the \gls{er} gene expression, and the tree diagram of the heirarchical clustering of the genes is shown on the left of the heatmap.
	}
...
For example, the \gls{er} pathway metagene was compared with \textit{ESR1} gene expression, and if the metagene was not in the same direction as \textit{ESR1} gene expression, the direction of the metagene was corrected (\cref{fig:methods/meta_dir}).
...
\begin{table}[htb]
	...
	\begin{center}
		\begin{tabular}{lcl}
			Pathway & Representative gene & Pathway description\\
			\hline
...
It was known from \citet{Gatza2010a} study that some pathways clustered and grouped together with specific pathways, so the direction of some of the metagenes were altered so that the grouping of the pathways matched the results of their study.
...
%TODO: re-write the linear model stuff:
